...
class FileItem {
	...
	async update(force = false) {
		...
		const promiseStats = this.promiseStats;
		this.promiseStats = new Promise((resolve/*, reject*/) => {
			promiseStats.then(() => {
				this.getNewStats().then(() => {
					resolve();
				});
			});
		});
...
class FileList {
	...
	async update(force = false) {
		...
		const promiseFileItems = this.promiseFileItems;
		this.promiseFileItems = new Promise((resolve/*, reject*/) => {
			promiseFileItems.then(() => {
				this.getNewFileItems().then(() => {
					resolve();
				});
			});
		});
