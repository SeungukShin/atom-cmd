...
class FileListView {
	 ...
	 * @property {NodeJS.Timer} updateTimer
	...
	getActive() {
		return this.fileList.getActive();
	}

	getSelectFileItems() {
		const fileItems = this.fileList.getSelectFileItems();
		if (fileItems.length == 0) {
			fileItems.push(this.fileList.getActiveFileItem());
		}
		return fileItems;
	}
	...
	pageUp() {
		...
		let active = prevActive - (this.table.getVisibleCount() - 2);
		if (active < 0) {
			active = 0;
		}
		this.fileList.setActive(active);
	 ...
	 * @param {Boolean} chdir
	 * @param {Boolean} updatedir
	 * @returns {Promise<void>}
	 */
	async update(newdir, chdir, updatedir) {
		// File list
		if (newdir || chdir) {
			while (1) {
				try {
		...
		if (newdir) {
			this.fileList.update(false);
			this.fileList.setActive(this.active);
		} else if (chdir || updatedir) {
			this.fileList.update(true);
		}
		...
		if (newdir || chdir) {
			...
			this.watcher = fs.watch(this.cwd, (event, filename) => {
				if (!this.updateTimer) {
